...
(() => {
  ...
  var categoryIndexByGroup = new Map(
    Object.entries(categoryOrderByGroup).map(
      ([group2, categories]) => [group2, new Map(categories.map((category, index) => [category, index]))]
    )
  );
  function compareQuestionsByStudyPath(left, right) {
    return compareGroup(left.group, right.group) || compareCategory(left.group, left.category, right.category) || left.difficulty - right.difficulty || numericId(left.id) - numericId(right.id) || compareText(left.id, right.id);
  }
  function sortCategoriesByStudyPath(group2, categories) {
    return [...categories].sort((left, right) => compareCategory(group2, left, right));
  }
  ...
  function compareCategory(group2, left, right) {
    if (left === right) return 0;
    const order = categoryIndexByGroup.get(group2);
    if (order) {
      const leftIndex = order.get(left);
      const rightIndex = order.get(right);
      const leftKnown = leftIndex !== void 0;
      const rightKnown = rightIndex !== void 0;
  ...
  function groupIndex(group2) {
    return groupIndexByOrder.get(group2) ?? Number.POSITIVE_INFINITY;
  }
  ...
  // src/application/sort-strategy.ts
  var StudyPathSortStrategy = class {
    sort(questions) {
      return [...questions].sort(compareQuestionsByStudyPath);
    }
  };

  // src/application/text-search.ts
  var asciiTechnicalTerm = /^[a-z0-9_:+.-]+$/i;
  var identifierCharacter = /^[a-z0-9_]$/i;
  function matchesSearchText(text2, rawQuery) {
    const query = rawQuery.trim().toLocaleLowerCase();
    if (!query) return true;
    const candidate = text2.toLocaleLowerCase();
  ...
  function startsAtIdentifierBoundary(query, before, after) {
    const needsLeftBoundary = identifierCharacter.test(query[0]);
    const needsRightBoundary = /^\d+$/u.test(query);
    return (!needsLeftBoundary || !identifierCharacter.test(before)) && (!needsRightBoundary || !identifierCharacter.test(after));
  }

  // src/application/question-query-service.ts
  var QuestionQueryService = class {
    constructor(repository, sortStrategy = new StudyPathSortStrategy()) {
      this.repository = repository;
      this.sortStrategy = sortStrategy;
    }
    query(state) {
      const query = state.query.trim();
      const matches = this.repository.findAll().filter((question) => (state.difficulty === null || question.difficulty === state.difficulty) && (state.group === "" || question.group === state.group) && (state.category === "" || question.category === state.category) && (state.scope === "" || question.scopes.includes(state.scope)) && matchesSearchText(searchableText(question), query));
  ...
  function isDifficultyValue(value) {
  ...
  var FilterState = class _FilterState {
    constructor(values = {}) {
      ...
      this.difficulty = values.difficulty == null ? null : Difficulty.from(values.difficulty).value;
    ...
    with(patch) {
      ...
      return new _FilterState({
        ...
        difficulty: patch.difficulty === void 0 ? this.difficulty : patch.difficulty,
        ...selection
      });
    }
  };
  function patchSelection(current, patch) {
    let group2 = patch.group ?? current.group;
    let category = patch.category ?? current.category;
    if (patch.group !== void 0 && patch.category === void 0) category = "";
    if (patch.category && patch.group === void 0) group2 = "";
    return { group: group2, category };
  }
  function normalizeSelection(group2, categoryValue) {
    const category = categoryValue.trim();
    if (!category) return { group: group2, category: "" };
    const separator = category.indexOf("/");
    if (separator <= 0 || !category.slice(separator + 1).trim()) {
      return { group: group2, category: "" };
    }
    const prefix = category.slice(0, separator);
    if (!questionGroups.has(prefix) || group2 && group2 !== prefix) {
      return { group: group2, category: "" };
    }
    return { group: prefix, category };
  }

  // src/application/question-state-store.ts
  var QuestionStateStore = class {
    constructor(initialState = new FilterState()) {
  ...
  function sameState(left, right) {
    return left.query === right.query && left.scope === right.scope && left.difficulty === right.difficulty && left.group === right.group && left.category === right.category;
  }
  ...
  function parseGroup(value) {
    ...
    return groups.has(candidate) ? candidate : "";
  }
  ...
  // src/infrastructure/browser-hash-adapter.ts
  var BrowserHashAdapter = class {
    constructor(location, history, hashAdapter = new HashStateAdapter()) {
  ...
  var text = z.string().trim().min(1);
  var httpUrl = text.url().refine(
    (value) => /^https?:\/\//i.test(value),
    "URL must use http or https"
  );
  var dateText = text.refine(isIsoDate, "Date must be a valid YYYY-MM-DD value");
  var group = z.enum(questionGroups2);
  var authority = z.enum(authorities);
  var answerSourceSchema = z.object({
    authority,
    topic: text,
  ...
  // src/infrastructure/browser-question-data-adapter.ts
  var BrowserQuestionDataAdapter = class {
    constructor(source) {
      this.source = source;
    }
    load() {
  ...
  function normalize(value) {
    ...
    const input = {
      ...
      ...value.pattern === void 0 ? {} : { pattern: value.pattern }
    };
    try {
  ...
  // src/infrastructure/in-memory-question-repository.ts
  ...
  // src/presentation/category-navigation-view.ts
  var CategoryNavigationView = class {
    constructor(document2) {
      this.items = [];
      const nav = document2.getElementById("category-nav");
  ...
  // src/presentation/filter-controls-view.ts
  var FilterControlsView = class {
    constructor(document2) {
      this.search = required(document2, "search-input");
      this.scope = required(document2, "scope-filter");
      this.difficulty = required(document2, "difficulty-filter");
      this.clear = required(document2, "clear-filters");
  ...
  function option(document2, value, label) {
    const element = document2.createElement("option");
    element.value = value;
    element.textContent = label;
    return element;
  }
  function required(document2, id) {
    const element = document2.getElementById(id);
    if (!element) throw new Error(`Missing filter element: #${id}`);
    return element;
  }

  // src/presentation/mobile-drawer-view.ts
  var MobileDrawerView = class {
    constructor(document2) {
      this.menu = required2(document2, "menu-button");
      this.drawer = required2(document2, "category-drawer");
      this.closeButton = required2(document2, "close-menu");
      this.scrim = required2(document2, "scrim");
    }
    bind() {
  ...
  function required2(document2, id) {
    const element = document2.getElementById(id);
    if (!element) throw new Error(`Missing drawer element: #${id}`);
    return element;
  }

  // src/presentation/question-acronym-glossary.ts
  var technicalTerms = [
  ...
  function buildTechnicalTermNotes(title) {
    return technicalTerms.map((entry) => ({ entry, index: title.indexOf(entry.term) })).filter(({ index }) => index >= 0).sort((left, right) => left.index - right.index || right.entry.term.length - left.entry.term.length).map(({ entry }) => `${entry.term} = ${entry.expansion}`);
  }

  // src/presentation/question-card-view.ts
  var QuestionCardView = class {
    constructor(document2) {
    ...
    render(question) {
      ...
      const glossary = glossaryNotes.length > 0 ? this.text("p", `\u672F\u8BED\u6CE8\u89E3\uFF1A${glossaryNotes.join("\uFF1B")}`, "question-glossary") : null;
  ...
  // src/presentation/question-list-view.ts
  var QuestionListView = class {
    constructor(document2, batchSize = 30, cards = new QuestionCardView(document2)) {
      this.batchSize = batchSize;
      this.cards = cards;
      this.questions = [];
      this.rendered = 0;
      this.list = required3(document2, "question-list");
      this.count = required3(document2, "result-count");
      this.empty = required3(document2, "empty-state");
    }
    render(questions) {
  ...
  function required3(document2, id) {
    const element = document2.getElementById(id);
    if (!element) throw new Error(`Missing presentation element: #${id}`);
    return element;
  }

  // src/controller/question-bank-controller.ts
  var QuestionBankController = class {
    constructor(repository, queryService, store, hash, views) {
      this.repository = repository;
    ...
    selectCategory(item) {
      ...
      else this.store.update({ group: item.group, category: item.id });
    }
    readHash() {
  ...
  function uniqueScopes(questions) {
  ...
  function buildNavigation(questions) {
    ...
    for (const group2 of studyPathGroupOrder) {
      const groupQuestions = questions.filter((question) => question.group === group2);
      items.push({
        type: "group",
        id: group2,
        group: group2,
        label: studyPathGroupLabels[group2],
  ...
  // src/app.ts
  function bootstrap(document2, source = globalThis, location = globalThis.location, history = globalThis.history) {
    ...
    const controls = new FilterControlsView(document2);
    const navigation = new CategoryNavigationView(document2);
    const list = new QuestionListView(document2);
    const drawer = new MobileDrawerView(document2);
    ...
    controller.registerCleanup(bindLoading(document2, controller));
    controller.registerCleanup(bindHashChange(document2, controller));
    return controller;
  }
  function bindLoading(document2, controller) {
    const sentinel = document2.getElementById("load-sentinel");
  ...
  function bindHashChange(document2, controller) {
  ...
  // src/main.ts
  function startWhenReady(document2) {
    let controller;
    const start = () => {
      controller ?? (controller = bootstrap(document2));
    };
    if (document2.readyState === "loading") {
      document2.addEventListener("DOMContentLoaded", start, { once: true });
    } else {
      start();
    }
    return () => {
      document2.removeEventListener("DOMContentLoaded", start);
      controller?.dispose();
    };
  }
  var stopApplication = startWhenReady(document);
})();
